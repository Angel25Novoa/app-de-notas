(function(){
    ...
    function renderTodos(){
        ...
        document.querySelectorAll('.todo button.editar').forEach(item => {
            item.addEventListener('click', e => {
                const id = e.target.parentNode.parentNode.getAttribute('data-id');
                const texto = getItemAndText(todos, {id: id});

                document.querySelector('itnew').value = nota;
    ...
    function getItemAndText(){
        ...
        localStorage.setItem('Texto-de-nota', texto);
        const nota = localStorage.getItem('Texto-de-nota');
        localStorage.removeItem('Texto-de-nota');
    }
